...
async def test_stop_game_by_creator_and_non_creator(application, store):
    ...
    user1 = await store.users.create_or_get_user({"id": 1, "username": "player1"})
    user1.state = State.idle

    user2 = await store.users.create_or_get_user({"id": 2, "username": "player2"})
    user2.state = State.idle

    application.telegram.send_message = AsyncMock()

    await play_command(application, chat_id=chat_id, user=user1, chat_type="group")
    ...
    await stop_game_command(application, chat_id=chat_id, user=user2, chat_type="group")
    ...
    await stop_game_command(application, chat_id=chat_id, user=user1, chat_type="group")
