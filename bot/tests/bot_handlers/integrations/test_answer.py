...
async def test_buzzer_answer(application, store):
    ...
    player1 = await store.users.create_or_get_user(
        {"id": 1, "username": "player1"}
    )
    player2 = await store.users.create_or_get_user(
    ...
    payload = {
        "text": "2+2?",
        "answers": [{"text": "4", "points": 10, "position": 1}],
    }
    ...
    application.round_service.get_current_buzzers = AsyncMock(
        return_value=allowed_ids
    )

    application.telegram.answer_callback_query = AsyncMock()

    await press_buzzer_callback(
        application, round_id, callback_id=101, user=player1
    )

    application.telegram.answer_callback_query.assert_awaited_with(
        101, "Вы первый!"
    )
    ...
    assert (
        round_in_db.current_buzzer_id is None
        or round_in_db.state == RoundState.team_play
    )
...
async def test_team_answer(application, store):
    ...
    players_team1 = [
        await store.users.create_or_get_user({"id": i, "username": f"p{i}"})
        for i in range(1, 6)
    ]
    ...
    opened_answers_json = await application.cache.pool.get(
