...
class Game(BaseModel):
    __tablename__ = "games"
    __table_args__ = (Index("ix_games_chat_status", "chat_id", "status"),)

    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    chat_id: Mapped[int] = mapped_column(BigInteger, nullable=False, index=True)
    status: Mapped[GameStatus] = mapped_column(
        Enum(GameStatus), default=GameStatus.created, nullable=False
    )
...
class Round(BaseModel):
    ...
    status: Mapped[RoundStatus] = mapped_column(
        Enum(RoundStatus), default=RoundStatus.waiting, nullable=False
    )
    started_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    ...
    def __repr__(self):
        return (
            f"<Round id={self.id} game_id={self.game_id} q={self.question_id}"
            f"#={self.round_number} status={self.status.value}>"
        )
